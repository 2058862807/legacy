...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await fetch(`${backendUrl}/api/live/accept?user_email=${encodeURIComponent(userEmail)}`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('Live accept backend error:', response.status, errorData)
    ...
    console.error('Live accept API error:', error)
